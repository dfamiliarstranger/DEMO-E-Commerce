...
from store.serializer import ProductSerializer, CategorySerializer, CartSerializer, CartOrderItem, CartOrderItemSerializer, CartOrderSerializer, CouponSerializer, NotificationSerializer, ReviewSerializer
...
class CartAPIView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if cart:
            ...
            service_fee_percentage = 2.5 / 100
            cart.service_fee = Decimal(service_fee_percentage) * cart.sub_total

            cart.total = cart.sub_total + cart.service_fee + cart.tax_fee + cart.shipping_amount 
            cart.save()
            ...
            cart.total = cart.sub_total + cart.service_fee + cart.tax_fee + cart.shipping_amount 
            cart.save()
            return Response({'message':"Cart Updated succesfully"}, status=status.HTTP_201_CREATED)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if session_id != 'null':
            ...
            if session.payment_status == 'paid':
                ...
                return Response({"message":"Payment Unsuccessful"})
